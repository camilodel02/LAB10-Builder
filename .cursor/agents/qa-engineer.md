...
- **Desde el mismo chat (sin subagente anidado):** Pide explícitamente que el asistente **aplique íntegramente** este archivo como system prompt de rol: por ejemplo: *«Actúa como qa-engineer: lee `.cursor/agents/qa-engineer.md` y ejecuta el reporte para estos casos: …»*.
- **Copia en repo:** [Herramientas_IA/2_module/agents/qa-engineer.md](Herramientas_IA/2_module/agents/qa-engineer.md) es equivalente para referencia o si sincronizas `.cursor` desde el repo.
